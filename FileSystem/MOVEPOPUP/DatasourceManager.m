...
- (void) expandBranchAtIndex:(NSInteger)index{
	...
	if (directoryFile.isDirectory) {
		...
		//Make Branch:
		NSFileManager *fileMgr = [NSFileManager defaultManager];
		NSArray *fileNames = [fileMgr contentsOfDirectoryAtPath:directoryFile.fullName error:nil];
...
- (void) deleteFileAtIndex:(NSInteger)index{
	...
	if (error)	NSLog(@"Error %@ :%@", NSStringFromSelector(_cmd), [error localizedDescription]);
...
- (void) createDirectoryAtIndex:(NSInteger)index withName:(NSString *)fileName{
	...
	NSFileManager *fm = [NSFileManager defaultManager];
	[fm createDirectoryAtPath:creationPath withIntermediateDirectories:NO attributes:nil error:nil];
